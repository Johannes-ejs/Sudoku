...
Game::Game(int level,int num_players): level(level), num_players(num_players){
    ...
    srand(time(nullptr));
    client=false;
    if(level == 0) {times_remove=1;}
    if(level == 1){times_remove=3;}
    if(level == 2){times_remove=10;}
    int M[9][9];
    reset_data(M);
    solve(0,0,M,true);
//    test_sol(); até aqui funciona
    int i,j;
    for(int m=0;m<9;m++) for(int n=0;n<9;n++)M[m][n]=soluction[m][n];
    ...
    if(level == 0) {times_add=num_zeros/10;}
    if(level == 1){times_add=num_zeros/20;}
    if(level == 2){times_add=0;}
    while(times_add--){
        do{
            i=rand()%9;
            j=rand()%9;
    ...
    if(soluction[i][j]==x){
        ...
        for(int m=0;m<9 && key;m++){
            for(int n=0;n<9 && key;n++){
                if(data_game[num_player][m][n]==0) key=0;
            }
        }
        if(key) {
            return 2;
...
void Game:: reset_data(int (&M)[9][9]){
    for(int i=0;i<9;i++) for(int k=0;k<9;k++) M[i][k]=0;
}
